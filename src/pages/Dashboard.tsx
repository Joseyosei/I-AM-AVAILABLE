...
export default function Dashboard() {
  ...
  const stats = [
    { icon: Eye, label: 'Profile Views', value: user.profileViews.toLocaleString(), change: '+12% this week' },
    { icon: MousePointerClick, label: 'Contact Clicks', value: user.contactClicks.toLocaleString(), change: '+8% this week' },
    { icon: Sparkles, label: 'Subscription Tier', value: user.tier.charAt(0).toUpperCase() + user.tier.slice(1), badge: user.tier },
  ];
  ...
  return (
    <DashboardLayout>
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-8">
        {/* Welcome Header */}
        <div>
          <h1 className="font-serif text-3xl font-bold mb-2">Welcome back, {user.name.split(' ')[0]}!</h1>
        ...
        {completeness < 100 && (
          ...
          >
            ...
            <p className="text-sm text-muted-foreground mb-3">
              A complete profile gets up to 5x more views. Add missing details to stand out.
            </p>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="bg-card border border-border rounded-lg p-6">
          ...
          <div className="flex flex-wrap gap-3">
            <Badge variant={user.availability === 'available' ? 'default' : 'secondary'} className="text-sm py-1.5">
              <span className={`w-2 h-2 rounded-full mr-2 ${
                user.availability === 'available' ? 'bg-green-400 animate-pulse'
                  : user.availability === 'open' ? 'bg-yellow-400' : 'bg-gray-400'
              }`} />
              {user.availability === 'available' ? 'Available Now' : user.availability === 'open' ? 'Open to Conversations' : 'Not Available'}
            </Badge>
            <Badge variant="outline" className="text-sm py-1.5">{user.tier.toUpperCase()} Tier</Badge>
            {user.featured && (
              <Badge variant="outline" className="text-sm py-1.5"><Sparkles className="w-3 h-3 mr-1" />Featured</Badge>
            )}
          </div>
        </motion.div>
