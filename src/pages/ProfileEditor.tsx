...
export default function ProfileEditor() {
  ...
  const canAddPortfolioLink = formData.portfolioLinks.length < tierLimits.portfolioLinks;
  ...
  const toggleOpenTo = (option: OpenToOption) => {
    const newOpenTo = formData.openTo.includes(option)
      ? formData.openTo.filter(o => o !== option)
      : [...formData.openTo, option];
    setFormData({ ...formData, openTo: newOpenTo });
  };
      ...
      >
          ...
          <div className="space-y-2">
            <div className="flex justify-between">
              ...
              <span className="text-xs text-muted-foreground">
                {formData.bio.length} / {tierLimits.bioLength}
              </span>
            </div>
            <Textarea
              id="bio"
              value={formData.bio}
              onChange={(e) => {
                if (e.target.value.length <= tierLimits.bioLength) {
        ...
        <section className="bg-card border border-border rounded-lg p-6">
          <div className="flex justify-between items-center mb-4">
            ...
            <span className="text-xs text-muted-foreground">
              {formData.skills.length} of {tierLimits.skills === Infinity ? '∞' : tierLimits.skills} skills ({user.tier.charAt(0).toUpperCase() + user.tier.slice(1)} tier)
            </span>
          </div>
          ...
          <div className="flex gap-2 mb-4">
            ...
            />
            <Button onClick={addSkill} disabled={!canAddSkill || !newSkill.trim()}>
        ...
        <section className="bg-card border border-border rounded-lg p-6">
          <div className="flex justify-between items-center mb-4">
            ...
            <span className="text-xs text-muted-foreground">
              {formData.portfolioLinks.length} of {tierLimits.portfolioLinks === Infinity ? '∞' : tierLimits.portfolioLinks} links ({user.tier.charAt(0).toUpperCase() + user.tier.slice(1)} tier)
            </span>
          </div>
          
          {tierLimits.portfolioLinks === 0 ? (
            <p className="text-sm text-muted-foreground">
              Upgrade to Pro to add portfolio links
            </p>
          ) : (
            <>
              <div className="flex gap-2 mb-4">
                <Input
                  ...
                  disabled={!canAddPortfolioLink}
                />
                <Button onClick={addPortfolioLink} disabled={!canAddPortfolioLink || !newPortfolioLink.trim()}>
                  <Plus className="w-4 h-4" />
                </Button>
              </div>
