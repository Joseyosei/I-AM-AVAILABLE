...
export default function Settings() {
  ...
  const tier = (profile?.tier || 'free') as keyof typeof planFeatures;
  ...
  const handleDeleteAccount = async () => {
    await logout();
    toast({ title: 'Signed out', description: 'Contact support to delete your account permanently.' });
  };
    ...
    <DashboardLayout>
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="max-w-2xl space-y-8">
        ...
        <section className="bg-card border border-border rounded-lg p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <Badge variant={tier === 'free' ? 'secondary' : 'default'}>
              {tier === 'premium' && <Sparkles className="w-3 h-3 mr-1" />}
              {tier.toUpperCase()}
            </Badge>
          </div>
          <p className="text-2xl font-bold mb-4">
            {tier === 'free' ? '$0' : tier === 'pro' ? '$29' : '$79'}
            <span className="text-base font-normal text-muted-foreground">/month</span>
          </p>
          <ul className="space-y-2 mb-6">
            {planFeatures[tier].map((feature) => (
              <li key={feature} className="flex items-center gap-2 text-sm"><Check className="w-4 h-4 text-primary" />{feature}</li>
            ))}
          </ul>
          {tier === 'free' ? (
            <Link to="/pricing"><Button className="w-full gap-2"><Sparkles className="w-4 h-4" />Upgrade Plan</Button></Link>
          ) : (
        ...
        <section className="bg-card border border-border rounded-lg p-6">
          ...
          <p className="text-sm text-muted-foreground mb-4">Current email: <span className="text-foreground font-medium">{user.email}</span></p>
          <div className="space-y-4">
            <div className="space-y-2"><Label htmlFor="newEmail">New Email</Label><Input id="newEmail" type="email" value={newEmail} onChange={(e) => setNewEmail(e.target.value)} placeholder="newemail@example.com" /></div>
            <Button onClick={handleUpdateEmail} disabled={!newEmail.trim() || isUpdatingEmail}>
              {isUpdatingEmail ? <><Loader2 className="w-4 h-4 mr-2 animate-spin" />Updating...</> : 'Update Email'}
            </Button>
          </div>
        </section>
        ...
        <section className="bg-card border border-border rounded-lg p-6">
          ...
          <div className="space-y-4">
            <div className="space-y-2"><Label htmlFor="newPassword">New Password</Label><Input id="newPassword" type="password" value={newPassword} onChange={(e) => setNewPassword(e.target.value)} placeholder="••••••••" /></div>
            <div className="space-y-2"><Label htmlFor="confirmPassword">Confirm Password</Label><Input id="confirmPassword" type="password" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} placeholder="••••••••" /></div>
            <Button onClick={handleUpdatePassword} disabled={!newPassword || !confirmPassword || isUpdatingPassword}>
        ...
        <section className="bg-card border border-destructive/30 rounded-lg p-6">
          <h2 className="font-serif text-lg font-semibold text-destructive mb-4 flex items-center gap-2"><AlertTriangle className="w-5 h-5" />Danger Zone</h2>
